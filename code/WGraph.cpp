...
void WGraph::save_graph_to_bin(string path, string file) {
    ofstream outfile(path + file + "/" + string("graph_10.bin"), ofstream::binary);
    if (!outfile) {
...
uint32_t WGraph::query_very_naive(uint32_t s, uint32_t t, uint32_t r, double& qtime) {
    auto t1 = std::chrono::high_resolution_clock::now();
    uint32_t min_dist = INVALID_VALUE;
    uint32_t i = 0, j = 0;

    while (i < labeling_naive[r][s].size() && j < labeling_naive[r][t].size()) {
        uint32_t curr_u = labeling_naive[r][s][i].first;
        uint32_t curr_w = labeling_naive[r][t][j].first;
        if (curr_u < curr_w) {
            i++;
        }
            ...
            uint32_t d1 = labeling_naive[r][s][i].second;
            uint32_t d2 = labeling_naive[r][t][j].second;
            if (d1 + d2 < min_dist) {
                min_dist = d1 + d2;
            }
            i++;
    ...
    while (i < labeling_naive[r][s].size()) {
        uint32_t curr_u = labeling_naive[r][s][i].first;
        if (curr_u == t) {
            uint32_t d1 = labeling_naive[r][s][i].second;
            if (d1 < min_dist) {
    ...
    while (j < labeling_naive[r][t].size()) {
        uint32_t curr_w = labeling_naive[r][t][j].first;
        if (curr_w == s) {
            uint32_t d2 = labeling_naive[r][t][j].second;
            if (d2 < min_dist) {
...
uint32_t WGraph::query_naive(uint32_t s, uint32_t t, uint32_t r, double& qtime) {
    auto t1 = std::chrono::high_resolution_clock::now();
    uint32_t min_dist = INVALID_VALUE;
    uint32_t i = 0, j = 0;
...
// default query algorithm
uint32_t WGraph::query_advanced(uint32_t s, uint32_t t, uint32_t r, double& qtime) {
    auto t1 = std::chrono::high_resolution_clock::now();
    uint32_t min_dist = INVALID_VALUE;
    uint32_t i = 0, j = 0;
    ...
    while (i < labeling_v[s].size() && j < labeling_v[t].size()) {
        ...
        if (curr_u < curr_w) {
            ...
            if (l1 != offset[s][i+1]) {
                uint16_t d1 = labeling_dw[s][l1].first;
                ...
                if (l2 != offset[t][j+1]) {
                    uint16_t d2 = labeling_dw[t][l2].first;
    ...
    while (i < labeling_v[s].size()) {
        ...
        if (curr_u == t) {
            uint32_t l1 = lower_bound(labeling_dw[s].begin() + offset[s][i], labeling_dw[s].begin() + offset[s][i + 1], make_pair(0,r), compare_w<pair<uint16_t, uint8_t>>) - labeling_dw[s].begin();
            if (l1 != offset[s][i+1]) {
                if (labeling_dw[s][l1].first < min_dist) {
                    min_dist = labeling_dw[s][l1].first;
                }
            }
        }
        i++;
    ...
    while (j < labeling_v[t].size()) {
        ...
        if (curr_w == s) {
            uint32_t l2 = lower_bound(labeling_dw[t].begin() + offset[t][j], labeling_dw[t].begin() + offset[t][j + 1], make_pair(0,r), compare_w<pair<uint16_t, uint8_t>>) - labeling_dw[t].begin();
            if (l2 != offset[t][j + 1]) {
                if (labeling_dw[t][l2].first < min_dist) {
                    min_dist = labeling_dw[t][l2].first;
                }
            }
        }
        j++;
...
int32_t WGraph::query_while_indexing_vertex(uint32_t s, uint32_t t, uint32_t r) {
    uint32_t min_dist = INVALID_VALUE;
    uint32_t i = 0, j = 0;
...
bool WGraph::query_while_indexing_vertex_plus(uint32_t s, uint32_t t, uint16_t curr_d, uint8_t r, vector<vector<pair<uint16_t, uint8_t>>>& u_label) {
...
void WGraph::build_naive_index() {
    labeling_naive = vector<vector<vector<pair<uint32_t, uint16_t>>>>(max_w+1, vector<vector<pair<uint32_t, uint16_t>>>(nsize));

    clock_t start, end;
    start = clock();

    for (uint32_t r = 0; r <= max_w; r++) {
        /// step 1: filter graph and leave all the weights that are larger than the constraint r
        vector<vector<uint32_t>> newg = vector<vector<uint32_t>>(nsize);
        for (uint32_t u = 0; u < nsize; u++) {
            for (uint32_t nidx = 0; nidx < ndegree[u]; nidx++) {
        ...
        for (uint32_t u = 0; u < nsize; u++) {
            ...
            vector<uint32_t> visited = vector<uint32_t>(nsize, INVALID_VALUE);
            visited[u] = 0;
            labeling_naive[r][u].emplace_back(make_pair(u, 0));
            while (!myqueue.empty()) {
                uint32_t v = myqueue.front();
                myqueue.pop();
                for (auto w : newg[v]) {
                    if (w > u && visited[w] == INVALID_VALUE) {  // use visited for both visited info and store disntance
                        visited[w] = visited[v] + 1;
                        labeling_naive[r][w].emplace_back(make_pair(u, visited[w]));
                        myqueue.emplace(w);
                    }
                }
            }
            if (u % 2000 == 0) {
                cout << u << " vertices finished (r = " << r << ")!" << endl;
                clock_t temp = clock();
...
void WGraph::build_index(string type, string graph_type) {
    ...
    if (type == "wc-index-plus") {
        ...
        test_query("wc-index-plus");
    }
    else if (type == "naive-index") {
        build_naive_index();
        test_query("naive-index");
    }
    end = clock();
    cout << "build index: " << (float)(end - start) / CLOCKS_PER_SEC << " s" << endl;
}
...
void WGraph::vertex_prioritized_indexing_plus(uint32_t u, boost::dynamic_bitset<>& updated, vector<uint8_t>& visited_r, boost::dynamic_bitset<>& this_visited_flag) {
    ...
    while (!Q1.empty()) {
        ...
        while (!Q1.empty()) {
            ...
            if (query_while_indexing_vertex_plus(u, v, curr_d, r, u_label)) {
                continue;
            }
            else {
                if (updated[v] & 1) {
...
void WGraph::get_index_size(string type) {
    if (type == "wc-index" || type == "wc-index-plus"){
        ...
        cout << "--------------Details of Index Occupation-------------" << endl;
        uint64_t cnt = 0;
        for (uint32_t w = 0; w < labeling_naive.size(); w++) {
            for (uint32_t u = 0; u < labeling_naive[w].size(); u++) {
                cnt += labeling_naive[w][u].size();
            }
        }
        cout << "total number of unsigned ints 32+16 in the index: " << cnt << endl;
    }
    else {
...
void WGraph::test_correctness() {
    random_device rd;
    std::mt19937 gen(rd());
    uniform_int_distribution<uint32_t> gen_w(0, max_w);

    double q1 = 0, q2 = 0, q3 =0, q4 = 0, q5 = 0, q6 = 0;

    uint32_t cnt = 0;
    for (uint32_t s = 0; s < nsize - 1; s++) {
        if (s % 200 == 0) cout << "testing s = " << s << endl;
        for (uint32_t t = s + 1; t < nsize; t++) {
            cnt++;
            uint32_t r = gen_w(gen);

            uint32_t d1 = constrained_shortest_distance_naive(s, t, r, q1);
            uint32_t d2 = constrained_shortest_distance_dijkstra(s, t, r, q2);
            uint32_t d3 = constrained_shortest_distance_plus(s, t, r, q3);
            //uint32_t d4 = query_very_naive(s, t, r, q4);
            uint32_t d4 = query_naive(s, t, r, q4);
            uint32_t d5 = query(s, t, r, q5);
            uint32_t d6 = query_advanced(s, t, r, q5);
            if (!(d1 == d2 && d2 == d3 && d3 == d6)) {
...
void WGraph::test_query(string version) {
    random_device rd;
    std::mt19937 gen(rd());
    uniform_int_distribution<uint32_t> gen_w(0, max_w);
    ...
    uniform_int_distribution<uint32_t> gen_s(0, nsize-1);
    uniform_int_distribution<uint32_t> gen_t_(0, nsize-1);
    int cnt = 100;
    for (int i = 0; i < cnt; i++) {
        cout << "performing query " << i << endl;
        uint32_t w = gen_w(gen);
        uint32_t s = gen_s(gen);
        uint32_t t = gen_t_(gen);
        if (version == "naive") {
            uint32_t d1 = constrained_shortest_distance_naive(s, t, w, q1);
            uint32_t d2 = 0; //constrained_shortest_distance_dijkstra(s, t, w, q2);
            uint32_t d3 = constrained_shortest_distance_plus(s, t, w, q3);
        }
        if (version == "wc-index-plus") {
            uint32_t d1 = query_naive(s, t, w, q1);
            uint32_t d2 = query(s, t, w, q2);
            uint32_t d3 = query_advanced(s, t, w, q3);
        }
        if (version == "naive-index") {
            uint32_t d4 = query_very_naive(s, t, w, q4);
        }
    }
...
// baseline
void WGraph::vertex_prioritized_indexing(uint32_t u, boost::dynamic_bitset<>& updated, vector<uint8_t>& visited_r) {
    vector<uint32_t> visited_vertex;
...
void WGraph::generate_random_weight() {
    ...
    std::uniform_int_distribution<> distr(1, weight);

    for (uint32_t u = 0; u < nsize; u++) {
        for (uint32_t v = 0; v < wlist[u].size(); v++) {
            wlist[u][v] = distr(gen);
        }
        if (u % 2000 == 0) cout << "finished u = " << u << endl;
    }

    //print_graph();

    cout << "Finished generating random weights." << endl;
}
